...
export function NewInvoiceModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Не удалось создать накладную");
    } finally {
  ...
  if (suppliers.length === 0) {
    return (
      <Modal title="Новая накладная" onClose={onClose}>
  ...
  return (
    <Modal title="Новая накладная" onClose={onClose} width="max-w-2xl">
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          ...
          <input
            ...
            placeholder="Как указано в бумажном документе"
            className="w-full rounded-xl border border-border bg-surface px-3 py-2 text-sm text-foreground outline-none focus:border-accent focus:ring-2 focus:ring-accent/20"
          />
        </div>
        ...
        <p className="mb-3 text-xs text-muted">
          Накладная сохранится как черновик. Остатки на складе изменятся только после того, как вы
          её проведёте.
        </p>
        ...
        >
          {isSubmitting ? "Сохранение…" : "Сохранить накладную"}
        </button>
      </form>
    </Modal>
  );
